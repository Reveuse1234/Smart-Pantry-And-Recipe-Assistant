...
def _candidate_venv_python_from(base: Path) -> Path | None:
    for candidate in (
...
def _resolve_venv_python(base_dir: Path | None = None) -> Path:
    base_dir = base_dir or REPO
    for candidate_base in (base_dir, *base_dir.parents):
        resolved = _candidate_venv_python_from(candidate_base)
